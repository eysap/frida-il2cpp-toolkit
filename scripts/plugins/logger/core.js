...
(function(global) {
  ...
  /**
   * Resolve UI config (prefer global config, fallback to plugin config)
   * @param {Object} config - Plugin config
   * @returns {Object} UI config
   */
  function getUiConfig(config) {
    const globalUi = global.IL2CPPToolkitConfig?.ui;
    if (globalUi && typeof globalUi === "object") return globalUi;
    return config?.ui || {};
  }
  ...
  function hookMethods(_klass, classFullName, methods, config) {
    ...
    const timer = setInterval(() => {
      if (idx >= methods.length || hooked >= (config.performance?.maxHooks ?? 300)) {
        clearInterval(timer);
      ...
      try {
        ...
        Interceptor.attach(addr, {
          onEnter: function(args) {
            ...
            const isVerbose = uiConfig.verbosity === 'verbose';

            if (config.logging?.args) {
              for (let i = 0; i < method.parameters.length; i++) {
                if (i >= config.logging?.maxArgs ?? 8) break;
            ...
            const collapseCfg = uiConfig.collapse;
    ...
    }, config.performance?.hookDelayMs ?? 25);
  }
